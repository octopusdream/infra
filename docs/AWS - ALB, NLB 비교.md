...
---
### 프로젝트에서 사용한 로드밸런서
본 프로젝트에서는 총 2개의 로드밸런서 NLB를 사용하였다.

부하 분산과 auto scaling을 위한 로드밸런서
- 외부용 NLB를 생성하고, NLB에 고정 IP(EIP)를 붙여 로드밸런서에 접근 할때 아이피나 DNS 둘다 사용이 가능할 수 있도록 하였다.
- NLB에 rule과 target group을 각 가용영역 별 worker node들로 설정하여 부하 분산으로 인한 auto scaling이 가능하도록 설정하였다. 

master node의 HA를 위한 로드밸런서
- 내부용 NLB를 생성하고 NLB에 rule과 target group을 각 가용영역 별 master node로 설정해 control plane(master node)를 최소 3개이상 운영함으로써 Highly Available(HA) cluster 운영한다.
